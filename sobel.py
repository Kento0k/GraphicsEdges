...
def main_func():
    ...
    draw_image(img_with_kx_mask, './out/sobel_out', '/sobel_kx.png')

    # Применение ky
    img_with_ky_mask = img_convolution(expanded_img, get_ky_mask())
    draw_image(img_with_ky_mask, './out/sobel_out', '/sobel_ky.png')

    # Сложение результатов проходов с kx и ky
    addition_img = img_addition(img_with_kx_mask, img_with_ky_mask)
    draw_image(addition_img, './out/sobel_out', '/sobel.png')
